...
class Postgres(Component):
    ...
    def and_super_password(self):
        self.__super_password = self.__authenticable.ask_password(constants['super_password'].format( self.__name, self.__super_password ), self.__super_password)
        return self

    def and_if_use_persistent_volume(self):
        self.__use_persistent_volume = self.__optional.ask_use(constants['use_persistent_volume'].format( self.__name , self.__use_persistent_volume))
        return self

    def and_volume_size(self):
        if self.__use_persistent_volume:
            self.__volume_size = self.__quantifiable.ask_quantity(constants['volume_size'].format( self.__name , self.__volume_size))
        return self
