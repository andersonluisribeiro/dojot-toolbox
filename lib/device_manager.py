...
class DeviceManager(Component):
    ...
    def __init__(self):
        ...
        self.__name = constants['devm_name']
        self.__authenticable = Authenticable()         
        self.__quantifiable = Quantifiable()

    def ask_pg_username(self):
        self.__pg_username = self.__authenticable.ask_username(constants['devm_pg_user'].format( self.__name, self.__pg_username ), self.__pg_username)
        return self

    def ask_pg_password(self):
        self.__pg_password = self.__authenticable.ask_password(constants['devm_pg_password'].format( self.__name, self.__pg_password ), self.__pg_password)
        return self   
